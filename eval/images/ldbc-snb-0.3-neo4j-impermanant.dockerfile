...
RUN wget -O maven.tar.gz -L https://dlcdn.apache.org/maven/maven-3/3.9.5/binaries/apache-maven-3.9.5-bin.tar.gz &&\
    ...
    wget -O social_network-csv_composite-longdateformatter-sf0.3.tar.gz -L https://utexas.box.com/shared/static/u6xwqnx9qr8w8lbzmsxvykieg94elkqf.gz &&\
    gzip -d social_network-csv_composite-longdateformatter-sf0.3.tar.gz &&\
    tar -xvf social_network-csv_composite-longdateformatter-sf0.3.tar && rm *.tar &&\
...
WORKDIR /social_network-csv_composite-longdateformatter-sf0.3
...
COPY --chown=$username ./estore/ldbc/snb/src/test/java/org/estore/eval/estore/ldbc/snb/Neo4jImpermanantTest03 /neo4j/community/community-it/kernel-it/src/test/java/org/neo4j/kernel/impl/core/Neo4jImpermanantTest03.java
