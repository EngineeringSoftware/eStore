...
RUN wget -O maven.tar.gz -L https://dlcdn.apache.org/maven/maven-3/3.9.5/binaries/apache-maven-3.9.5-bin.tar.gz &&\
    ...
    wget -O social_network-csv_composite-longdateformatter-sf0.1.tar.gz -L https://utexas.box.com/shared/static/p2uqletspnkr0cev6a6ohtd7zwx7chuf.gz &&\
    gzip -d social_network-csv_composite-longdateformatter-sf0.1.tar.gz &&\
    tar -xvf social_network-csv_composite-longdateformatter-sf0.1.tar && rm *.tar &&\
...
WORKDIR /social_network-csv_composite-longdateformatter-sf0.1
...
COPY --chown=$username ./estore/ldbc/snb/src/test/java/org/estore/eval/estore/ldbc/snb/Neo4jImpermanantTest01 /neo4j/community/community-it/kernel-it/src/test/java/org/neo4j/kernel/impl/core/Neo4jImpermanantTest01.java
