...
public class InGraphReflectionTestArrayTable100000 {
  ...
  public void setupData() throws Exception {
    ...
    for (long j = 0; j < 320; j++) {
      rows.add(j);
      columns.add(j);
    }
    table = ArrayTable.create(rows, columns);
    for (long j = 0; j < 320; j++) {
      for (long k = 0; k < 320; k++) {
  ...
  public void testFindElement() {
    long ind = rand.nextLong(0, 320);
    long ind2 = rand.nextLong(0, 320);
    ...
    assertEquals(
        ...
        table.get(ind, ind2),
        "Mismatch in IngraphReflectionTestArrayTable100000 for testFindElement");
  }
}
