...
public class EstoreNeoTestArrayTable100000 {
  ...
  public void setupData() throws Exception {
    ...
    for (long j = 0; j < 316; j++) {
      rows.add(j);
      columns.add(j);
    }
    table = ArrayTable.create(rows, columns);
    for (long j = 0; j < 316; j++) {
      for (long k = 0; k < 316; k++) {
  ...
  public void testFindElement() throws Exception {
    long ind = rand.nextLong(0, 316);
    long ind2 = rand.nextLong(0, 316);
    ...
    assertEquals(
        ((Long) result[0]),
        table.get(ind, ind2),
        "Mismatch in EstoreNeoTestArrayTable100000 for testFindElement");
  }
}
