package org.estore.eval.ldbc.snb;
...
public class Snb01Test {
    ...
    public void testInteractiveShortQuery1() {
        ...
        Table result =
                estore.query(
                        "MATCH (n:`org.estore.eval.ldbc.snb.util.Person`"
                                + " {id:32985348833679})-[:IS_LOCATED_IN]->(p:`org.estore.eval.ldbc.snb.util.Place`)"
    ...
    public void readDataSet() throws Exception {
        ...
        String datasetPath =
                "src/test/java/org/estore/eval/ldbc/data/social_network-csv_composite-longdateformatter-sf0.1";
    ...
    private void insertComments(String filePath, HashMap<Long, Comment> comments) {
        try {
            ...
            for (CSVRecord csvRecord : csvParser) {
                ...
                comments.put(
                        id,
                        new Comment(id, creationDate, locationIP, browserUsed, content, length));
            }
            csvParser.close();
        } catch (Exception e) {
            e.printStackTrace();
